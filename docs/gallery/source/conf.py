...
try:
    ...
    GIT["release"] = release = (
        check_output(["git", "describe", "--tags", "--always"]).decode().strip()
    )
...
sphinx_gallery_conf = {
    "examples_dirs": [
        "examples/vision/guides/test_examples",
...
def generate_colab_url(notebook_path: str) -> t.Optional[str]:
    ...
    notebook_path = (
        notebook_path if notebook_path.endswith(".ipynb") else notebook_path + ".ipynb"
    )
    ...
    template = "https://colab.research.google.com/github/{user}/{repo}/blob/{branch}/{notebook_path}"
...
def generate_binder_url(notebook_path: str) -> t.Optional[str]:
    ...
    template = "https://mybinder.org/v2/gh/{user}/{repo}/{branch}?labpath={filepath}"
...
def get_check_example_api_reference(filepath: str) -> t.Optional[str]:
    ...
    mapping = {"tabular": tabular_check_clazz, "vision": vision_check_clazz}
    if (
        type not in mapping
        or mapping[type] is None
        or not hasattr(mapping[type], "__module__")
    ):
        return
