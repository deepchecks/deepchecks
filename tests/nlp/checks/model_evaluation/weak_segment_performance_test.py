...
def test_simple_test(tweet_emotion_train_test_textdata):
    # Arrange
    ...
    assert_that(condition_result, has_items(
        equal_condition_result(is_pass=False,
                               details='Found a segment with Accuracy score of 0.667 in comparison to an average score of 0.84 in sampled data.',
                               name='The relative performance of weakest segment is greater than 80% of average model performance.')
    ))

    assert_that(result.value['avg_score'], close_to(0.84, 0.001))
    assert_that(len(result.value['weak_segments_list']), equal_to(4))
    assert_that(result.value['weak_segments_list'].iloc[0,0], close_to(0.66, 0.01))
