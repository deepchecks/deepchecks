...
from mlchecks.checks.integrity.invalid_chars import invalid_characters
...
def test_single_column_no_invalid():
    ...
    # Act
    result = invalid_characters(dataframe)
    # Assert
...
def test_double_column_ignored_invalid():
    ...
    dataframe = pd.DataFrame(data=data)
    # Act
    result = invalid_characters(dataframe, ignore_columns=['col1'])
    # Assert
    assert_that(result.value, has_length(0))
...
def test_double_column_specific_invalid():
    ...
    dataframe = pd.DataFrame(data=data)
    # Act
    result = invalid_characters(dataframe, columns=['col1'])
    # Assert
    assert_that(result.value, has_length(1))
...
def test_double_column_specific_and_ignored_invalid():
    ...
    dataframe = pd.DataFrame(data=data)
    # Act & Assert
    assert_that(calling(invalid_characters).with_args(dataframe, ignore_columns=['col1'], columns=['col1']),
