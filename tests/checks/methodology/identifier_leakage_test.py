...
def test_assert_identifier_leakage():
    ...
    result = IdentifierLeakage().run(dataset=Dataset(df, label_name='label', date_name='x2', index_name='x3'))
    print(result.value)
...
def test_assert_identifier_leakage_class():
    ...
    result = identifier_leakage_check.run(dataset=Dataset(df, label_name='label', date_name='x2', index_name='x3'))
    print(result.value)
    for key, value in result.value.items():
        assert_that(key, is_in(expected.keys()))
        assert_that(value, close_to(expected[key], 0.1))
...
def test_nan():
    ...
    result = IdentifierLeakage().run(dataset=Dataset(nan_df, label_name='label', date_name='x2', index_name='x3'))
    for key, value in result.value.items():
        assert_that(key, is_in(expected.keys()))
        assert_that(value, close_to(expected[key], 0.1))
...
def test_condition_pps_pass():
    ...
    # Act
    result = check.conditions_decision(check.run(Dataset(df, label_name='label', date_name='x2', index_name='x3')))
