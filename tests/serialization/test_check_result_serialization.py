...
from deepchecks.core.serialization.check_result.widget import (
    ...
    DisplayItemsHandler as WidgetDisplayItemsHandler,
)
...
def test_html_serialization_with__output_id__parameter():
    ...
    assert_that(
        output,
        all_of(instance_of(str), has_length(greater_than(0)), contains_string(output_id)),
    )
...
def test_html_serialization_to_full_html_page():
    ...
    regexp = (
        rf"^{whitespace}{anything}<html>{whitespace}"
        rf'<head><meta charset="utf-8"\/><\/head>{whitespace}'
        rf"<body{anything}>{anything}<\/body>{whitespace}"
        rf"<\/html>{whitespace}$"
    )
...
def test_display_map_serialization_to_html():
    html_section = HtmlDisplayItemsHandler.handle_display(
        display=[DisplayMap(a=create_check_result_display())], include_header=False, include_trailing_link=False
    )
...
def test_nested_display_map_serialization_to_list_of_ipython_formatters():
    ...
    assert_that(
        ...
        all_of(
            ...
            has_length(
                equal_to(13)
            ),  # three section headers + ten display items created by create_check_result_display
            only_contains(instance_of_ipython_formatter()),
        ),
    )
                ...
                has_entries(
                    {
                        ...
                        "payload": has_entries({"a": all_of(instance_of(list), has_length(equal_to(5)))}),
                    }
                )
            ),
        ),
                    ...
                    has_entries(
                        {
                            ...
                            "b": all_of(
                                ...
                                contains_exactly(
                                    serialized_to_json_displaymap(
                                        has_entries({"a": all_of(instance_of(list), has_length(equal_to(5)))})
                                    )
                                ),
                            ),
                        }
            ...
            has_entries(
                {
                    ...
                    "conditions_results": all_of(
                        instance_of(list),
                        has_length(greater_than(0)) if with_conditions_table is True else has_length(equal_to(0)),
                    ),
                    "display": all_of(
                        instance_of(list),
...
def test_display_map_serialization_to_widget():
    ...
    output = WidgetDisplayItemsHandler.handle_display(
        display=[displaymap], include_header=False, include_trailing_link=False
    )
    assert_that(
        output, contains_exactly(serialized_to_widget_displaymap(displaymap=displaymap, display_items=display_items))
    )
...
def test_nested_display_map_serialization_to_widget():
    ...
    output = WidgetDisplayItemsHandler.handle_display(
        display=[outer_displaymap], include_header=False, include_trailing_link=False
    )
    assert_that(
        output,
        contains_exactly(
            serialized_to_widget_displaymap(
                ...
                tabs=[
                    ...
                    contains_exactly(
                        serialized_to_widget_displaymap(  # tab 'b'
                            displaymap=inner_displaymap, display_items=display_items
                        )
                    ),
                ],
            )
...
def serialized_to_widget_displaymap(
    displaymap: DisplayMap, display_items: t.Optional[t.List[t.Any]] = None, tabs: t.Optional[t.List[t.Any]] = None
):
    if display_items is not None:
        tabs_matcher = [
            all_of(instance_of(VBox), has_property("children", has_length(len(display_items))))
    ...
    return all_of(
        ...
        has_property(
            ...
            contains_exactly(
                instance_of(HTML),
                all_of(instance_of(Tab), has_property("children", all_of(contains_exactly(*tabs_matcher)))),
            ),
        ),
    )
...
def assert_widget_output(output, check_result, with_conditions_section=True, with_display_section=True):
    ...
    assert_that(
        output,
        all_of(instance_of(VBox), has_property("children", all_of(instance_of(tuple), has_length(children_count)))),
    )

    assert_that(
        output.children[header_section_index],
        all_of(
            instance_of(HTML), has_property("value", all_of(instance_of(str), contains_string(check_result.header)))
        ),
    )
    assert_that(
    ...
    if with_display_section is True:
        assert_that(
            ...
            all_of(
                instance_of(VBox),
                has_property(
                    "children", all_of(instance_of(tuple), has_length(equal_to(len(check_result.display) + 1)))
                ),
            ),  # plus header element
        )
