...
def test_pycocotools_stuff(coco_test_visiondata: VisionData, trained_yolov5_object_detection):
    ...
    s = res['precision'][:, 3, 2, :]
    print(np.mean(s[s>-1]))
    # assert_that(), close_to(0.38, 0.04))
    assert_that(np.mean(res['precision'][0, 3, 2, :]), close_to(0.51, 0.01))
    ...
    assert_that(res['recall'][-1]['all'][1][0], close_to(0.3, 0.01))
    ...
    assert_that(res['recall'][-1]['large'][100][0], close_to(0.53, 0.01))