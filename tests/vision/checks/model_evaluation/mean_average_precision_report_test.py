...
def test_mnist_error(mnist_visiondata_test, mock_mnist_model, device):
    # Arrange
    check = MeanAveragePrecisionReport()
    # Act
    assert_that(
        calling(check.run
                ).with_args(mnist_visiondata_test, mock_mnist_model,
                            device=device),
        raises(ModelValidationError, r'Check is irrelevant for task of type TaskType.CLASSIFICATION')
    )


def test_coco(coco_visiondata_test, mock_trained_yolov5_object_detection, device):
    # Arrange
    check = MeanAveragePrecisionReport() \
...
def test_coco_area_param(coco_visiondata_test, mock_trained_yolov5_object_detection, device):
    # Arrange
...
def test_coco_area_param_without_display(coco_visiondata_test, mock_trained_yolov5_object_detection, device):
    # Arrange
    check = MeanAveragePrecisionReport(area_range=(40**2, 100**2))
