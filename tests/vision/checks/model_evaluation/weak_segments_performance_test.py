...
def test_detection_condition(coco_visiondata_train, mock_trained_yolov5_object_detection, device):
...
def test_classification_defaults(mnist_visiondata_train, mock_mnist_model, device):
    ...
    # Act
    result = check.run(mnist_visiondata_train, mock_mnist_model, device=device, n_samples=1000)

    # Assert
    assert_that(result.value['avg_score'], close_to(0.067, 0.001))


def test_segmentation_defaults(segmentation_coco_visiondata_test, trained_segmentation_deeplabv3_mobilenet_model, device):
    check = WeakSegmentsPerformance()

    assert_that(calling(check.run).with_args(
        segmentation_coco_visiondata_test, trained_segmentation_deeplabv3_mobilenet_model, device),
