...
def test_equal_pycocotools(coco_test_visiondata: VisionData, trained_yolov5_object_detection):
    ...
    assert_that(metric.get_val_at(res['precision'], iou=0.75, area='all', max_dets=100), close_to(0.376, 0.01))
    assert_that(metric.get_val_at(res['precision'], area='small', max_dets=100), close_to(0.189, 0.01))
    assert_that(metric.get_val_at(res['precision'], area='medium', max_dets=100), close_to(0.367, 0.001))
    assert_that(metric.get_val_at(res['precision'], area='large', max_dets=100), close_to(0.476, 0.01))
    ...
    assert_that(metric.get_val_at(res['recall'], area='all', max_dets=100), close_to(0.388, 0.01))
