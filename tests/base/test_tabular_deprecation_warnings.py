...
def test_deprecation_segment_performance_warning():
    with pytest.warns(
        DeprecationWarning,
        match="The SegmentPerformance check is deprecated and will be removed in the "
...
def test_deprecation_whole_dataset_drift_warning():
    with pytest.warns(
        DeprecationWarning,
        match="The WholeDatasetDrift check is deprecated and will be removed in the "
...
def test_deprecation_systematic_regression_warning():
    with pytest.warns(
        DeprecationWarning,
        match="RegressionSystematicError check is deprecated and will be removed in "
...
def test_deprecation_label_type_dataset():
    with pytest.warns(
        DeprecationWarning,
        match="regression_label value for label type is deprecated, allowed task "
...
def test_deprecation_y_pred_train_single_dataset():
    ds = Dataset(
        pd.DataFrame(
            {"a": np.random.randint(0, 5, 50), "b": np.random.randint(0, 5, 50), "label": np.random.randint(0, 2, 50)}
        ),
        label="label",
    )
...
def test_deprecation_y_pred_test_single_dataset():
    ...
    with pytest.warns(DeprecationWarning, match="y_pred_test is deprecated and ignored."):
        _ = WeakSegmentsPerformance().run(
            ds, y_pred=y_pred_train, y_proba=y_proba_train, y_pred_test=y_pred_train, y_proba_test=y_proba_train
        )
...
def test_deprecation_category_mismatch_train_test():
    with pytest.warns(
        DeprecationWarning, match="CategoryMismatchTrainTest is deprecated, use NewCategoryTrainTest " "instead"
    ):
...
def test_deprecation_warning_train_test_prediction_drift():
    with pytest.warns(
        DeprecationWarning,
        match="The TrainTestPredictionDrift check is deprecated and will be removed"
...
def test_deprecation_warning_train_test_feature_drift():
    with pytest.warns(
        DeprecationWarning,
        match="The TrainTestFeatureDrift check is deprecated and will be removed in"
...
def test_deprecation_warning_train_test_label_drift():
    with pytest.warns(
        DeprecationWarning,
        match="The TrainTestLabelDrift check is deprecated and will be removed in "
