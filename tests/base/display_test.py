...
def test_check_result_deserialization_from_json(iris):
    ...
    json_to_display = jsonpickle.dumps(
        {
            "type": "CheckResult",
            "check": {"type": "DummyCheckClass", "name": "Dummy Check", "summary": "summary", "params": {}},
            "display": [
                {"type": "html", "payload": "hi"},
                {"type": "plotly", "payload": plot.to_json()},
                {"type": "dataframe", "payload": pd.DataFrame({"a": [1, 2], "b": [1, 2]}).to_dict(orient="records")},
            ],
            "header": "header",
            "value": 5,
        }
...
def test_check_result_show_with_sphinx_gallery_env_enabled():
    with plotly_default_renderer("sphinx_gallery"):
        ...
        assert_that(r._repr_html_(), all_of(instance_of(str), has_length(greater_than(0)), is_html_document()))
...
def test_check_result_repr_html():
    ...
    result = create_check_result(value=10)
    # Assert
    assert_that(result._repr_html_(), all_of(instance_of(str), has_length(greater_than(0)), is_html_document()))
...
def test_check_result_repr_json():
    ...
    # Assert
    assert_that(result._repr_json_(), all_of(instance_of(dict), has_length(greater_than(0))))
...
def test_check_failure_repr_mimebundle():
    ...
    assert_that(
        ...
        all_of(
            ...
            has_length(greater_than(0)),
            has_entries(
                {"text/html": instance_of(str), "application/json": any_of(instance_of(dict), instance_of(dict))}
            ),
        ),
    )
...
def test_check_failure_repr_html():
    ...
    failure = CheckFailure(DummyCheck(), Exception("error message"))
    # Assert
    assert_that(failure._repr_html_(), all_of(instance_of(str), has_length(greater_than(0))))
...
def test_check_failure_repr_json():
    ...
    failure = CheckFailure(DummyCheck(), Exception("error message"))
    # Assert
    assert_that(failure._repr_json_(), all_of(instance_of(dict), has_length(greater_than(0))))
...
def test_suite_result_repr_mimebundle():
    ...
    assert_that(
        ...
        all_of(
            ...
            has_length(greater_than(0)),
            has_entries(
                {"text/html": instance_of(str), "application/json": any_of(instance_of(dict), instance_of(list))}
            ),
        ),
    )
...
def test_suite_result_repr_html():
    ...
    suite_result = create_suite_result()
    # Assert
    assert_that(suite_result._repr_html_(), all_of(instance_of(str), has_length(greater_than(0)), is_html_document()))
...
def test_suite_result_repr_json():
    ...
    suite_result = create_suite_result()
    # Assert
    assert_that(suite_result._repr_json_(), all_of(instance_of(dict), has_length(greater_than(0))))
...
def is_html_document():
    ...
    regexp = (
        rf'^{any_whitespace}({anything})<html( lang="en")*>{any_whitespace}'
        rf"<head>({anything})<\/head>{any_whitespace}"
        rf"<body({anything})>({anything})<\/body>{any_whitespace}"
        rf"<\/html>{any_whitespace}$"
    )
