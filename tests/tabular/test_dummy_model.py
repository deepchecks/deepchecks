...
def test_new_data(diabetes_split_dataset_and_model):
    ...
    # Act
    assert_that(
        calling(NewDataCheck().run).with_args(
            train_dataset=train,
            test_dataset=test,
            y_pred_train=y_pred_train,
            y_pred_test=y_pred_test,
...
def test_bad_pred_shape(diabetes_split_dataset_and_model):
    ...
    # Act
    assert_that(
        calling(RegressionErrorDistribution().run).with_args(dataset=test, y_pred=y_pred_train),
        raises(ValidationError, r"Prediction array expected to be of same length as data 146, but was: 296"),
    )
...
def test_bad_pred_proba(iris_labeled_dataset, iris_adaboost):
    ...
    # Act
    assert_that(
        calling(RocReport().run).with_args(dataset=iris_labeled_dataset, y_pred=y_pred_train, y_proba=y_proba_train),
        raises(ValidationError, r"Prediction probabilities expected to be of length 150 but was: 149"),
    )
