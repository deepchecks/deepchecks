...
def test_classification_string_labels(iris_labeled_dataset):
    ...
    iris_labeled_dataset = Dataset(
        iris_labeled_dataset.data.replace(replace_dict), label=iris_labeled_dataset.label_name
    )
...
def test_classification_deepchecks_scorers(iris_split_dataset_and_model):
    ...
    train, test, model = iris_split_dataset_and_model
    check = TrainTestPerformance(
        scorers=["fpr_per_class", "fnr_per_class", "specificity_per_class", "fnr_macro", "roc_auc_per_class"]
    )
    # Act
