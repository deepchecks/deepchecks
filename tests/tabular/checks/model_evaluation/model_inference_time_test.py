...
def test_model_inference_time_check(iris_split_dataset_and_model: t.Tuple[Dataset, Dataset, object]):
    ...
    details_pattern = (
        r"Average model inference time for one sample \(in seconds\): " rf"{SCIENTIFIC_NOTATION_REGEXP.pattern}"
    )
...
def test_model_inference_time_check_with_condition_that_should_pass(
    ...
    details_pattern = re.compile(rf"Found average inference time \(seconds\): {SCIENTIFIC_NOTATION_REGEXP.pattern}")
    assert_that(
        condition_result,
        equal_condition_result(  # type: ignore
            is_pass=True, category=ConditionCategory.PASS, name=name, details=details_pattern
        ),
    )
...
def test_model_inference_time_check_with_condition_that_should_not_pass(
    ...
    assert_that(
        condition_result,
        equal_condition_result(  # type: ignore
            is_pass=False, category=ConditionCategory.FAIL, name=name, details=details_pattern
        ),
    )
