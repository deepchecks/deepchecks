...
def test_no_drift_regression_label_emd(diabetes, diabetes_model):
    ...
    # Assert
    assert_that(
        result.value, has_entries({"Drift score": close_to(0.04, 0.01), "Method": equal_to("Earth Mover's Distance")})
    )
...
def test_no_drift_regression_label_ks(diabetes, diabetes_model):
    ...
    # Assert
    assert_that(
        result.value, has_entries({"Drift score": close_to(0.11, 0.01), "Method": equal_to("Kolmogorov-Smirnov")})
    )
...
def test_drift_regression_label_raise_on_proba(diabetes, diabetes_model):
    ...
    # Act & Assert
    assert_that(
        calling(check.run).with_args(train, test, diabetes_model),
        raises(DeepchecksValueError, 'probability_drift="proba" is not supported for regression tasks'),
    )
...
def test_drift_max_drift_score_condition_fail_psi(drifted_data_and_model):
    ...
    check = PredictionDrift(
        categorical_drift_method="PSI", drift_mode="prediction"
...
def test_balance_classes_without_correct_drift_mode():
    # Arrange
    assert_that(
        calling(PredictionDrift).with_args(balance_classes=True, drift_mode="proba"),
        raises(
            DeepchecksValueError,
...
def test_balance_classes_with_drift_mode_auto(drifted_data):
    ...
    assert_that(
        ...
        has_entries(
            ...
            }  # If cramer's V then proves it changed to prediction mode
        ),
    )
...
def test_drift_max_drift_score_condition_pass_threshold(drifted_data_and_model):
    ...
    assert_that(
        condition_result,
        equal_condition_result(
            is_pass=True, details="Found model prediction PSI drift score of 0.79", name="Prediction drift score < 1"
        ),
    )
...
def test_binary_proba_condition_fail_threshold(drifted_data_and_model):
    ...
    test = remove_label(test)
    check = PredictionDrift(
        categorical_drift_method="PSI", numerical_drift_method="EMD", drift_mode="proba"
    ).add_condition_drift_score_less_than()
    ...
    # Assert
    assert_that(
        result.value, has_entries({"Drift score": close_to(0.23, 0.01), "Method": equal_to("Earth Mover's Distance")})
    )
...
def test_multiclass_proba_reduce_aggregations(iris_split_dataset_and_model_rf):
    ...
    assert_that(
        ...
        has_entries(
            {
                "Drift Score class 0": close_to(0.06, 0.01),
                "Drift Score class 1": close_to(0.06, 0.01),
