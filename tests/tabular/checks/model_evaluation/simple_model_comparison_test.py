...
def test_dataset_wrong_input():
    ...
    # Act & Assert
    assert_that(
        calling(SimpleModelComparison().run).with_args(bad_dataset, bad_dataset, None),
...
def test_classification_binary_string_labels_custom_scorer(iris_binary_string_split_dataset_and_model):
    ...
    assert_that(
        ...
        equal_to(
            {
                "scores": {
                    "f1_score": {"Origin": 0.9411764705882353, "Simple": 0.0},
                    "recall_score": {
                        "a": {"Origin": 0.9411764705882353, "Simple": 1.0},
...
def test_condition_pass_for_multiclass_avg_with_classes(iris_split_dataset_and_model):
    ...
    check = (
        ...
        .add_condition_gain_greater_than(1, average=False)
...
def test_classification_tree_custom_metric(iris_split_dataset_and_model):
    ...
    check = SimpleModelComparison(
        strategy="tree",
        scorers={"recall": make_scorer(recall_score, average=None), "f1": make_scorer(f1_score, average=None)},
    )
    # Act X
    result = check.run(train_ds, test_ds, clf).value
    # Assert
