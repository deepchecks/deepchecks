...
def test_unused_feature_detection(iris_split_dataset_and_model_rf):
    ...
    # Assert
    assert_that(
        set(result.value["used features"]), equal_to({"petal width (cm)", "petal length (cm)", "sepal length (cm)"})
    )
...
def test_low_feature_importance_threshold(iris_split_dataset_and_model_rf):
    ...
    # Assert
    assert_that(
        set(result.value["used features"]),
        equal_to({"petal width (cm)", "petal length (cm)", "sepal width (cm)", "sepal length (cm)"}),
    )
