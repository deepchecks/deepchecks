...
def test_no_drift(drifted_data):
    ...
    train_ds = Dataset(
        train_ds.data.drop(columns=["numeric_with_drift", "categorical_with_drift"]), label=train_ds.label_name
    )
    test_ds = Dataset(
        test_ds.data.drop(columns=["numeric_with_drift", "categorical_with_drift"]), label=test_ds.label_name
    )
        ...
        has_entries(
            {
                ...
                "domain_classifier_feature_importance": has_entries(
                    {"categorical_without_drift": close_to(0.81, 0.001), "numeric_without_drift": close_to(0.2, 0.02)}
                ),
            }
        ),
    )
