...
def test_drift_with_model(drifted_data_and_model):
    ...
    check = FeatureDrift(
        categorical_drift_method="PSI", numerical_drift_method="EMD", max_num_categories=10, min_category_size_ratio=0
    )
        ...
        has_entries(
            {
                ...
                "categorical_without_drift": has_entries(
                    {"Drift score": close_to(0, 0.01), "Method": equal_to("PSI"), "Importance": close_to(0, 0.01)}
                ),
                "categorical_with_drift": has_entries(
                    {"Drift score": close_to(0.22, 0.01), "Method": equal_to("PSI"), "Importance": close_to(0, 0.01)}
                ),
            }
        ),
    )
...
def test_drift_with_model_n_top(drifted_data_and_model):
    ...
    check = FeatureDrift(
        ...
        columns=["categorical_with_drift"],
        n_top_columns=1,
        ...
        has_entries(
            {
                ...
                "categorical_without_drift": has_entries(
                    {"Drift score": close_to(0, 0.01), "Method": equal_to("PSI"), "Importance": equal_to(None)}
                ),
                "categorical_with_drift": has_entries(
                    {"Drift score": close_to(0.22, 0.01), "Method": equal_to("PSI"), "Importance": equal_to(None)}
                ),
            }
        ),
    )
        ...
        has_entries(
            {
                ...
                "categorical_without_drift": has_entries(
                    {"Drift score": close_to(0.04, 0.01), "Method": equal_to("PSI"), "Importance": equal_to(None)}
                ),
                "categorical_with_drift": has_entries(
                    {"Drift score": close_to(0.24, 0.01), "Method": equal_to("PSI"), "Importance": equal_to(None)}
                ),
            }
        ),
    )
...
def test_drift_with_nulls_not_enough_samples_in_numerical_columns(drifted_data_with_nulls):
    ...
    assert_that(
        result.display[3],
        equal_to(
            "<span>The following columns do not have enough samples to calculate drift score: "
...
def test_aggregation_drift_with_model(drifted_data_and_model):
    ...
    assert_that(
        aggregated_result_l3["L5 Weighted Drift Score"], greater_than(aggregated_result_l2["L3 Weighted Drift Score"])
    )
    assert_that(aggregated_result_l1.keys(), has_item("Weighted Drift Score"))
    assert_that(
        aggregated_result_l2["L3 Weighted Drift Score"], greater_than(aggregated_result_l1["Weighted Drift Score"])
    )
...
def test_drift_max_drift_score_condition_fail_cramer_and_ks(drifted_data_and_model):
    ...
    train, test, model = drifted_data_and_model
    check = FeatureDrift(
        categorical_drift_method="cramers_v", numerical_drift_method="KS"
    ).add_condition_drift_score_less_than()
...
def test_drift_max_drift_score_condition_pass_threshold(drifted_data_and_model):
    ...
    check = FeatureDrift(
        categorical_drift_method="PSI", numerical_drift_method="EMD", max_num_categories=10, min_category_size_ratio=0
    ).add_condition_drift_score_less_than(max_allowed_categorical_score=1, max_allowed_numeric_score=1)
