...
def test_single_col_mismatch_without_display():
    ...
    # Act
    result = StringMismatchComparison().run(
        pd.DataFrame(data=data), pd.DataFrame(data=compared_data), with_display=False
    )
...
def test_condition_no_new_variants_pass():
    ...
    assert_that(
        result,
        equal_condition_result(
            is_pass=True, details="Passed for 1 relevant column", name="No new variants allowed in test data"
        ),
    )
...
def test_condition_percent_new_variants_fail():
    ...
    # Act
    test_df, base_df = pd.DataFrame(data=tested_data), pd.DataFrame(data=base_data)
    result, *_ = check.conditions_decision(
        check.run(
            base_df,
            test_df,
        )
