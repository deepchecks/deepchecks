...
def test_drift_regression_label_emd(drifted_regression_label):
    ...
    # Assert
    assert_that(
        result.value, has_entries({"Drift score": close_to(0.34, 0.01), "Method": equal_to("Earth Mover's Distance")})
    )
...
def test_drift_regression_label_ks(drifted_regression_label):
    ...
    # Assert
    assert_that(
        result.value, has_entries({"Drift score": close_to(0.71, 0.01), "Method": equal_to("Kolmogorov-Smirnov")})
    )
...
def test_drift_max_drift_score_condition_fail_psi(drifted_classification_label):
    ...
    train, test = drifted_classification_label
    check = LabelDrift(
        categorical_drift_method="PSI", numerical_drift_method="EMD"
    ...
    assert_that(
        ...
        equal_condition_result(
            is_pass=False, name="Label drift score < 0.15", details="Label's drift score PSI is 0.24"
        ),
    )
...
def test_drift_max_drift_score_condition_pass_threshold(non_drifted_classification_label):
    ...
    assert_that(
        condition_result,
        equal_condition_result(
            is_pass=True, details="Label's drift score PSI is 3.37E-3", name="Label drift score < 1"
        ),
    )
