...
def test_single_column_explicit_mix():
    ...
    assert_that(
        result.value,
        has_entries({"col1": has_entries({"strings": close_to(0.66, 0.01), "numbers": close_to(0.33, 0.01)})}),
    )
...
def test_double_column_specific_mix():
    ...
    assert_that(
        result.value, has_entries(col1=has_entries({"strings": close_to(0.66, 0.01), "numbers": close_to(0.33, 0.01)}))
    )
...
def test_condition_pass_fail_single_column():
    ...
    assert_that(
        ...
        has_items(
            equal_condition_result(
                is_pass=False,
                name="Rare data types in column are either more than 40% or less " "than 1% of the data",
        ...
        has_entries(
            {
                "col1": has_entries(
                    {
                        "strings": close_to(0.5, 0.01),
                        "numbers": close_to(0.5, 0.01),
