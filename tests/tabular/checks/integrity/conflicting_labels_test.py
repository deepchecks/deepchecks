...
def test_label_ambiguity():
    ...
    # Assert
    assert_that(
        result.value, has_entries({"percent_of_conflicting_samples": equal_to(1), "samples_indices": instance_of(list)})
    )
...
def test_label_ambiguity_empty():
    ...
    # Assert
    assert_that(
        result.value, has_entries({"percent_of_conflicting_samples": equal_to(0), "samples_indices": has_length(0)})
    )
...
def test_label_ambiguity_mixed():
    ...
    assert_that(
        result.value,
        has_entries({"percent_of_conflicting_samples": close_to(0.5, 0.01), "samples_indices": has_length(1)}),
    )
