...
def test_label_ambiguity_condition():
    ...
    ds = Dataset(pd.DataFrame(data), label='label')
    check = ConflictingLabels().add_condition_ratio_of_conflicting_labels_less_than_or_equal()
    ...
    assert_that(condition_result, has_items(
        equal_condition_result(is_pass=False,
                               ...
                               name='Ambiguous sample ratio is less than or equal to 0%')
    ))
...
def test_label_ambiguity_condition_pass():
    ...
    ds = Dataset(pd.DataFrame(data), label='label')
    check = ConflictingLabels().add_condition_ratio_of_conflicting_labels_less_than_or_equal(.7)
    ...
    assert_that(condition_result, has_items(
        equal_condition_result(is_pass=True,
                               details='Ratio of samples with conflicting labels: 50%',
                               name='Ambiguous sample ratio is less than or equal to 70%')
    ))
