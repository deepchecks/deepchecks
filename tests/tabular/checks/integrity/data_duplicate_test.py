...
def test_data_duplicates_ignore_index_column():
    duplicate_data = pd.DataFrame(
        {"col1": [1, 2, 1, 2, 1, 2, 1, 2, 1, 2], "col2": [1, 2, 1, 2, 1, 2, 1, 2, 1, 2], "col3": list(range(10))}
    )
...
def test_condition():
    ...
    assert_that(
        ...
        has_items(
            equal_condition_result(
                is_pass=True, details="Found 0% duplicate data", name="Duplicate data ratio is less or equal to 0%"
            )
        ),
    )
