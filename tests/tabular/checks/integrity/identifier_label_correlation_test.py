...
def test_assert_identifier_label_correlation():
    ...
    result = IdentifierLabelCorrelation().run(
        dataset=Dataset(df, label="label", datetime_name="x2", index_name="x3", cat_features=[])
    )
...
def test_assert_identifier_label_correlation_without_display():
    df, expected = generate_dataframe_and_expected()
    result = IdentifierLabelCorrelation().run(
        dataset=Dataset(df, label="label", datetime_name="x2", index_name="x3", cat_features=[]), with_display=False
    )
    for key, value in result.value.items():
        assert_that(key, is_in(expected.keys()))
        assert_that(value, close_to(expected[key], 0.1))
...
def test_dataset_only_label():
    ...
    assert_that(
        calling(IdentifierLabelCorrelation().run).with_args(dataset=df),
        raises(DatasetValidationError, "Dataset does not contain an index or a datetime"),
    )
...
def test_assert_label_correlation_class():
    ...
    result = identifier_leakage_check.run(
        dataset=Dataset(df, label="label", datetime_name="x2", index_name="x3", cat_features=[])
    )
    for key, value in result.value.items():
        assert_that(key, is_in(expected.keys()))
...
def test_nan():
    ...
    result = IdentifierLabelCorrelation().run(
        dataset=Dataset(nan_df, label="label", datetime_name="x2", index_name="x3", cat_features=[])
    )
    for key, value in result.value.items():
        assert_that(key, is_in(expected.keys()))
        assert_that(value, close_to(expected[key], 0.1))
...
def test_condition_pps_pass():
    ...
    # Act
    result = check.conditions_decision(
        check.run(Dataset(df, label="label", datetime_name="x2", index_name="x3", cat_features=[]))
    )
