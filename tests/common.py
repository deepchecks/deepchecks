...
def create_suite_result(
    ...
    results = [create_check_result(value=i, header=f"Dummy Result {i}") for i in range(n_of_results)]
    ...
    failures = [CheckFailure(DummyCheck(), Exception(f"Exception Message {i}")) for i in range(n_of_failures)]
    return SuiteResult(name=name, results=[*results, *failures], extra_info=["Some extra info regarding suite"])


def create_check_result(
    value: t.Any = None, header: str = "Dummy Result", include_display: bool = True, include_conditions: bool = True
) -> CheckResult:
    display = [*create_check_result_display(), DisplayMap(a=create_check_result_display())] if include_display else None
    result = CheckResult(
        value=value or 1000,
        header=header,
        display=display,
...
def assert_class_performance_display(
    ...
    pairs = ((dataset, metric) for dataset in ("Train", "Test") for metric in metrics)
...
def get_expected_results_length(suite: BaseSuite, args: t.Dict[t.Any, t.Any]):
...
def validate_suite_result(result: SuiteResult, min_length: int, exception_matcher: t.Optional[Matcher] = None):
    ...
    for check_result in result.results:
        if isinstance(check_result, CheckResult) and check_result.have_conditions():
            for cond in check_result.conditions_results:
                assert_that(
                    cond.category, any_of(ConditionCategory.PASS, ConditionCategory.WARN, ConditionCategory.FAIL)
                )
