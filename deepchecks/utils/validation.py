...
supported_models_html = f'<a href="{supported_models_link}" target="_blank">Supported Models</a>'
...
def model_type_validation(model: t.Any):
    ...
    if not isinstance(model, BasicModel):
        raise errors.ModelValidationError(
            'Model supplied does not meets the minimal interface requirements. For more info read the docs about '
            f'{supported_models_html}'
        )
