...
def model_error_contribution(
    ...
    error_model, new_feature_order = create_error_regression_model(
        numeric_features, categorical_features, random_state=random_state
    )
    ...
    if error_model_score < min_error_model_score:
        raise DeepchecksProcessError(
            f"Unable to train meaningful error model " f"(r^2 score: {format_number(error_model_score)})"
        )
    error_fi, _ = _calculate_feature_importance(
        error_model,
        Dataset(test_dataset, test_scores, cat_features=categorical_features),
        model_classes=None,
        observed_classes=None,
        task_type=TaskType.REGRESSION,
        permutation_kwargs={"random_state": random_state, "skip_messages": True},
    )
    ...
    for feature in error_fi.keys()[:max_features_to_show]:
        ...
        if feature in dataset.cat_features:
            ...
            error_per_segment_ser = (
                data.groupby(feature).agg(["mean", "count"])[error_col_name].sort_values("mean", ascending=not scorer)
            )
            ...
            if scorer:
                ok_name_feature, segment1_details = get_segment_details(
                    model, scorer, dataset, data[feature].isin(ok_categories)
                )
            else:
                ok_name_feature, segment1_details = get_segment_details_using_error(
                    error_col_name, data, data[feature].isin(ok_categories)
                )
            ...
            if with_display:
                ...
                if len(weak_categories) >= 1:
                    if scorer:
                        weak_name_feature, segment2_details = get_segment_details(
                            model, scorer, dataset, data[feature].isin(weak_categories)
                        )
                    else:
                        weak_name_feature, segment1_details = get_segment_details_using_error(
                            error_col_name, data, data[feature].isin(weak_categories)
                        )
                ...
                replace_dict = {
                    x: weak_name_feature if x in weak_categories else ok_name_feature
                    for x in error_per_segment_ser.index
                }
                ...
                display.append(
                    px.violin(
                        ...
                        y=error_col_name,
                        x=feature,
                        title=f"Segmentation of error by feature: {feature}",
                        box=False,
            ...
            tree_partitioner = DecisionTreeRegressor(
                max_depth=1, min_samples_leaf=min_segment_size + np.finfo(float).eps, random_state=random_state
            ...
            if len(tree_partitioner.tree_.threshold) > 1:
                ...
                if scorer:
                    segment1_text, segment1_details = get_segment_details(
                        model, scorer, dataset.copy(sampled_dataset), color_col
                    )
                    segment2_text, segment2_details = get_segment_details(
                        model, scorer, dataset.copy(sampled_dataset), ~color_col
                    )
...
def get_segment_details(
    model: Any, scorer: Callable, dataset: tabular.Dataset, segment_condition_col: pd.Series
...
def get_segment_details_using_error(
    error_column_name, dataset: pd.DataFrame, segment_condition_col: pd.Series
) -> Tuple[str, Dict[str, float]]:
