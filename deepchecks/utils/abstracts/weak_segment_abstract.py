...
class WeakSegmentAbstract(abc.ABC):
    ...
    def _target_encode_categorical_features_fill_na(
        self, data: pd.DataFrame, label_col: pd.Series, cat_features: List[str], is_cat_label: bool = True
    ) -> Dataset:
        ...
        for col in cat_features:
            categories_to_mask = [
                k
                for k, v in df_aggregated[col].value_counts().items()
                if v / data.shape[0] < self.categorical_aggregation_threshold
            ]
        ...
        while len(display_tabs.keys()) < self.n_to_show and idx + 1 < len(weak_segments):
            ...
            for f1_idx in range(len(segments_f1) - 1):
                for f2_idx in range(len(segments_f2) - 1):
                    segment_data = data[
                        np.asarray(feature1.between(segments_f1[f1_idx], segments_f1[f1_idx + 1]))
                        * np.asarray(feature2.between(segments_f2[f2_idx], segments_f2[f2_idx + 1]))
                    ]
                    if segment_data.empty:
                        ...
                        if scorer is not None and dummy_model is not None and label_col is not None:
                            scores[f2_idx, f1_idx] = scorer.run_on_data_and_label(
                                dummy_model, segment_data.drop(columns=temp_col), segment_data[temp_col]
                            )
                        else:
            ...
            msg = (
                f"Check ran on {data.shape[0]} data samples. {score_title} on the full data set "
    ...
    def _weak_segments_search(
        self,
        data: pd.DataFrame,
        score_per_sample: pd.Series,
        label_col: Optional[pd.Series] = None,
        feature_rank_for_search: Optional[np.ndarray] = None,
        dummy_model: Optional[_DummyModel] = None,
        scorer: Optional[DeepcheckScorer] = None,
        scorer_name: Optional[str] = None,
        multiple_segments_per_feature: bool = False,
    ) -> pd.DataFrame:
        ...
        for i in range(n_features):
            for j in range(i + 1, n_features):
                ...
                weak_segment_score, weak_segment_filter = self._find_weak_segment(
                    data, [feature1, feature2], score_per_sample, label_col, dummy_model, scorer
                )
        ...
        for _, row in weak_segments.sort_values(score_title).iterrows():
            ...
            if self.max_categories_weak_segment is not None and row["Feature1"] in self.encoder_mapping:
                unique_values_in_range = [
                    x
                    for x in self.encoder_mapping[row["Feature1"]]["encoded_value"].values
                    if row["Feature1 Range"][1] > x > row["Feature1 Range"][0]
                ]
                if len(unique_values_in_range) > self.max_categories_weak_segment:
                    ...
                    if dummy_model is not None and label_col is not None and scorer is not None:
                        new_row[score_title] = scorer.run_on_data_and_label(
                            dummy_model, subset, label_col[list(subset.index)]
                        )
                    else:
                        new_row[score_title] = score_per_sample[list(subset.index)].mean()

            if (
                self.max_categories_weak_segment is not None
                and new_row["Feature2"] != ""
                and row["Feature2"] in self.encoder_mapping
            ):
                unique_values_in_range = [
                    x
                    for x in self.encoder_mapping[row["Feature2"]]["encoded_value"].values
                    if row["Feature2 Range"][1] > x > row["Feature2 Range"][0]
                ]
                if len(unique_values_in_range) > self.max_categories_weak_segment:
                    subset = data.loc[new_row["Samples in Segment"]]
    ...
    def _find_weak_segment(
        self,
        data: pd.DataFrame,
        features_for_segment: List[str],
        score_per_sample: pd.Series,
        label_col: Optional[pd.Series] = None,
        dummy_model: Optional[_DummyModel] = None,
        scorer: Optional[DeepcheckScorer] = None,
        ...
        grid_searcher = GridSearchCV(
            ...
            scoring=neg_worst_segment_score,
    ...
    def _format_partition_vec_for_display(
        self, partition_vec: np.array, feature_name: str, seperator: Union[str, None] = "<br>"
    ) -> List[Union[List, str]]:
        ...
        for idx, segment in weak_segments_df.iterrows():
            for feature in ["Feature1", "Feature2"]:
                if segment[feature] in cat_features:
                    weak_segments_output[f"{feature} Range"][idx] = self._format_partition_vec_for_display(
                        segment[f"{feature} Range"], segment[feature], None
                    )[0]
    ...
    def add_condition_segments_relative_performance_greater_than(self, max_ratio_change: float = 0.20):
        ...
        def condition(result: Dict) -> ConditionResult:
            ...
            msg = (
                f'Found a segment with {scorer_name} of {format_number(weakest_segment_score, 3)} '
        ...
        return self.add_condition(
            f"The relative performance of weakest segment is greater than "
            f"{format_percent(1 - max_ratio_change)} of average model performance.",
            condition,
        )
