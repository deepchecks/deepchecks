...
def run_confusion_matrix_check(
    y_pred: np.ndarray, y_true: np.ndarray, with_display=True, normalize_display=True
) -> CheckResult:
...
def create_confusion_matrix_figure(
    confusion_matrix_data: np.ndarray, classes_names: List[str], normalize_display: bool
):
    ...
    confusion_matrix_norm = (
        ...
        / (confusion_matrix_data.sum(axis=1)[:, np.newaxis] + np.finfo(float).eps)
        * 100
    )
    ...
    if min(accuracy_array) < 100:
        display_msg += (
            f"<br>Best accuracy achieved on samples with <b>{classes_names[np.argmax(accuracy_array)]}"
            f"</b> label ({np.max(accuracy_array)}%)."
        )
        display_msg += (
            f"<br>Worst accuracy achieved on samples with <b>{classes_names[np.argmin(accuracy_array)]}"
    ...
    percent_data_each_cell = np.round(
        np.divide(np.nan_to_num(confusion_matrix_data, nan=0.0), total_samples) * 100, decimals=2
    )
    percent_data_each_col = (
        confusion_matrix_data.astype("float")
        / (confusion_matrix_data.sum(axis=0)[:, np.newaxis] + np.finfo(float).eps)
        * 100
    ).round(decimals=2)
...
def misclassified_samples_lower_than_condition(
    value: pd.DataFrame, misclassified_samples_threshold: float
) -> ConditionResult:
    ...
    if n_cells_above_thresh > 0:
        ...
        details = (
            f"Detected {n_cells_above_thresh} misclassified confusion matrix cell(s) each one "
    ...
    details = (
        "All misclassified confusion matrix cells contain less than "
