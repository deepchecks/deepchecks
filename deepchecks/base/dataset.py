...
class Dataset:
    ...
    Args:
        ...
        label (t.Union[Hashable, pd.Series, pd.DataFrame, np.ndarray])
            label column provided as name in the dataframe or label object which will be concatenated to the dataframe
    ...
    def __init__(
        ...
        if label is None:
            ...
            if isinstance(label, (pd.Series, pd.DataFrame, np.ndarray)):
                ...
                # Get label name
                if isinstance(label, (pd.Series, pd.DataFrame)) and label.name is not None:
