...
def load_data(
    ...
    if not as_train_test:
        ...
        if data_format == "Dataset":
            dataset = Dataset(
                dataset, label=_target, cat_features=_CAT_FEATURES, index_name=_index_name, datetime_name=_datetime_name
            )
            return dataset
        ...
        if data_format == "Dataset":
            train = Dataset(
                train, label=_target, cat_features=_CAT_FEATURES, index_name=_index_name, datetime_name=_datetime_name
            )
            test = Dataset(
                test, label=_target, cat_features=_CAT_FEATURES, index_name=_index_name, datetime_name=_datetime_name
            )
            return train, test
        elif data_format == "Dataframe":
            return train, test
        ...
        steps=[
            (
                ...
                run_available_kwargs(
                    OrdinalEncoder, handle_unknown="use_encoded_value", unknown_value=np.nan, dtype=np.float64
                ),
            )
        ]
    )
        ...
        steps=[
            ...
            (
                ...
                run_available_kwargs(
                    ...
                    max_iter=200,
                    random_state=42,
                    categorical_features=[False] * len(_NUM_FEATURES) + [True] * len(_CAT_FEATURES),
                ),
            ),
        ]
    )
