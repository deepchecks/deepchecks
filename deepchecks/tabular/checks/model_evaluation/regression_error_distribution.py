...
class RegressionErrorDistribution(SingleDatasetCheck):
    ...
    def __init__(
        self, n_top_samples: int = 3, n_bins: int = 40, n_samples: int = 1_000_000, random_state: int = 42, **kwargs
    ):
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        if context.with_display:
            ...
            n_largest = pd.concat(
                [dataset.data.loc[n_largest_diff.index], y_pred.loc[n_largest_diff.index], n_largest_diff], axis=1
            )

            n_smallest_diff = diff.nsmallest(self.n_top_samples)
            n_smallest_diff.name = str(dataset.label_name) + " Prediction Difference"
            n_smallest = pd.concat(
                [dataset.data.loc[n_smallest_diff.index], y_pred.loc[n_smallest_diff.index], n_smallest_diff], axis=1
            )
            fig = px.histogram(
                x=diff.values,
                nbins=self.n_bins,
