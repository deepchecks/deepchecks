...
class RocReport(SingleDatasetCheck):
    ...
    def __init__(self, excluded_classes: List = None, n_samples: int = 1_000_000, random_state: int = 42, **kwargs):
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        if not hasattr(context.model, "predict_proba"):
            raise DeepchecksNotSupportedError(
                "Predicted probabilities not supplied. The roc report check "
        ...
        for i, class_name in enumerate(dataset_classes):
            if self.excluded_classes is not None and class_name in self.excluded_classes:
                continue
            fpr[class_name], tpr[class_name], thresholds[class_name] = sklearn.metrics.roc_curve(
                dataset.label_col == class_name, y_pred_prob[:, i]
            )
        ...
        if context.with_display:
            ...
            fig.add_trace(
                go.Scatter(
                    x=[0, 1], y=[0, 1], line=dict(color="#444"), line_width=2, line_dash="dash", showlegend=False
                )
            )
    ...
    def add_condition_auc_greater_than(self, min_auc: float = 0.7):
        ...
        def condition(result: Dict) -> ConditionResult:
            failed_classes = {
                class_name: format_number(score) for class_name, score in result.items() if score <= min_auc
            }
            if failed_classes:
                return ConditionResult(
                    ConditionCategory.FAIL, f"Found classes with AUC below threshold: {failed_classes}"
                )
            else:
...
def get_cutoff_figure(tpr, fpr, thresholds, class_name):
    ...
    hovertemplate = (
        f"Class: {class_name}"
        + "<br>TPR: %{y:.2%}<br>FPR: %{x:.2%}"
        + f"<br>Optimal Threshold: {thresholds[highest_youden_index]:.3}"
    )
    return go.Scatter(
        x=[fpr[highest_youden_index]],
        y=[tpr[highest_youden_index]],
        mode="markers",
        marker_size=15,
