...
class WeakSegmentsPerformance(SingleDatasetCheck, WeakSegmentAbstract):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if loss_per_sample is not None and score_per_sample is None:
            warnings.warn(
                f"{self.__class__.__name__}: loss_per_sample is deprecated. " f"Please use score_per_sample instead.",
                DeprecationWarning,
            )
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        encoded_dataset = self._target_encode_categorical_features_fill_na(
            features_data, dataset.label_col, dataset_subset.cat_features, context.task_type != TaskType.REGRESSION
        )
        ...
        if self.score_per_sample is not None:
            ...
            if context.task_type == TaskType.REGRESSION:
                ...
                score_per_sample = calculate_neg_cross_entropy_per_sample(
                    dataset.label_col, y_proba, context.model_classes
                )

            dummy_model = _DummyModel(
                test=encoded_dataset, y_pred_test=predictions, y_proba_test=y_proba, validate_data_on_predict=False
            )
        ...
        # Running the logic
        weak_segments = self._weak_segments_search(
        ...
        if len(weak_segments) == 0:
            raise DeepchecksProcessError(
                "WeakSegmentsPerformance was unable to train an error model to find weak "
                "segments. Try increasing n_samples or supply additional features."
            )

        if context.with_display:
            display = self._create_heatmap_display(
                data=encoded_dataset.data,
        ...
        display_msg = (
            "Showcasing intersections of features with weakest detected segments.<br> The full list of "
