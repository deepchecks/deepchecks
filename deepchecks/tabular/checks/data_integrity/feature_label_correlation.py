...
class FeatureLabelCorrelation(SingleDatasetCheck):
    ...
    def add_condition_feature_pps_less_than(self: FLC, threshold: float = 0.8) -> FLC:
        ...
        def condition(value: t.Dict[Hashable, float]) -> ConditionResult:
            ...
            if failed_features:
                message = (
                    f"Found {len(failed_features)} out of {len(value)} features with PPS above threshold: "
                    f"{failed_features}"
                )
        ...
        return self.add_condition(
            f"Features' Predictive Power Score is less than {format_number(threshold)}", condition
        )
