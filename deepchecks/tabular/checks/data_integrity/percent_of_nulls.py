...
class PercentOfNulls(SingleDatasetCheck, ReduceFeatureMixin):
    ...
    def run_logic(self, context: Context, dataset_kind: DatasetKind) -> CheckResult:
        ...
        result_data = pd.DataFrame(
            data=result_data, columns=["Column", "Percent of nulls in sample", "Feature importance"]
        ).set_index(["Column"])
        ...
        if context.with_display and max(result_data["Percent of nulls in sample"]) > 0:
            display = [
                px.bar(
                    x=data.columns,
                    y=result_data["Percent of nulls in sample"],
                    title="Percent Of Nulls",
                    range_y=(0, 1),
    ...
    def add_condition_percent_of_nulls_not_greater_than(self, threshold: float = 0.05) -> TPercentOfNulls:
        ...
        def condition(result: pd.DataFrame) -> ConditionResult:
            failing = result[result["Percent of nulls in sample"] > threshold]["Percent of nulls in sample"].apply(
                format_percent
            )
            if len(failing) > 0:
                return ConditionResult(
                    ConditionCategory.FAIL,
                    f"Found {len(failing)} columns with ratio " f"of nulls above threshold: \n{dict(failing)}",
                )
            else:
