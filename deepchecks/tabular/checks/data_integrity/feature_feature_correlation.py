...
class FeatureFeatureCorrelation(SingleDatasetCheck):
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        if context.with_display:
            ...
            fig = [
                px.imshow(top_n_df, color_continuous_scale=px.colors.sequential.thermal),
                "* Displayed as absolute values.",
            ]
            if num_nans:
                fig.append(
                    f"* NaN values (where the correlation could not be calculated)"
                    f" are displayed as 0.0, total of {num_nans} NaNs in this display."
                )
            if len(dataset.features) > len(all_features):
                fig.append(
                    "* Some features in the dataset are neither numerical nor categorical and therefore not "
                    "calculated."
                )
        else:
            fig = None
        ...
        def condition(result):
            ...
            if len(high_corr_pairs) > n_pairs:
                return ConditionResult(
                    ConditionCategory.FAIL, f"Correlation is greater than {threshold} for pairs {high_corr_pairs}"
                )
            else:
                return ConditionResult(
                    ConditionCategory.PASS, f"All correlations are less than {threshold} except pairs {high_corr_pairs}"
                )
