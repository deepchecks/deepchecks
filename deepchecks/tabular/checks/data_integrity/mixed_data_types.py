...
class MixedDataTypes(SingleDatasetCheck):
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        for column_name in df.columns:
            ...
            if context.with_display and mix:
                ...
                formated_mix["Numbers examples"] = (
                    "[" + format_list([format_number(float(num)) for num in mix["numbers_examples"]]) + "]"
                )
    ...
    def _check_mixed_percentage(self, column_data: pd.Series) -> dict:
        ...
        return {
            ...
            "numbers_examples": numbers_in_col,
        }
    ...
    def add_condition_rare_type_ratio_not_in_range(self, ratio_range: Tuple[float, float] = (0.01, 0.1)):
        ...
        def condition(result):
            ...
            if failing_columns:
                details = (
                    f"Found {len(failing_columns)} out of {len(result)} columns with non-negligible quantities "
            ...
            details = (
                f"{len(result)} columns passed: found {len(result) - len(no_mix_columns)} columns with "
                f"negligible types mix, and {len(no_mix_columns)} columns without any types mix"
            )
            return ConditionResult(ConditionCategory.PASS, details)

        name = (
            f"Rare data types in column are either more than {format_percent(ratio_range[1])} or less "
