...
class FeatureLabelCorrelationChange(TrainTestCheck):
    ...
    def run_logic(self, context: Context) -> CheckResult:
        ...
        ret_value, display = get_feature_label_correlation(
            train_df,
    ...
    def add_condition_feature_pps_difference_less_than(
        self: FLC, threshold: float = 0.2, include_negative_diff: bool = True
    ) -> FLC:
        ...
        def condition(value: t.Dict[Hashable, t.Dict[Hashable, float]]) -> ConditionResult:
            ...
            if failed_features:
                message = (
                    f"Found {len(failed_features)} out of {len(diff_dict)} features with PPS difference above "
        ...
        return self.add_condition(
            f"Train-Test features' Predictive Power Score difference is less than " f"{format_number(threshold)}",
            condition,
        )
    ...
    def add_condition_feature_pps_in_train_less_than(self: FLC, threshold: float = 0.7) -> FLC:
        ...
        def condition(value: t.Dict[Hashable, t.Dict[Hashable, float]]) -> ConditionResult:
            ...
            if failed_features:
                message = (
                    f'Found {len(failed_features)} out of {len(value["train"])} features in train dataset with '
        ...
        return self.add_condition(
            f"Train features' Predictive Power Score is less than " f"{format_number(threshold)}", condition
        )
