...
class NewCategoryTrainTest(TrainTestCheck, ReduceFeatureMixin):
    ...
    def run_logic(self, context: Context) -> CheckResult:
        ...
        feature_importance = (
            pd.Series(index=list(cat_features), dtype=object)
            if context.feature_importance is None
        ...
        for feature in cat_features:
            ...
            if new_category_values:
                ...
                result_data.append(
                    [
                        feature,
                        len(new_category_values),
                        new_categories_ratio,
                        list(sorted_new_categories.keys()),
        ...
        if context.with_display:
            ...
            display["New Categories Names"] = display["New categories"].apply(
                lambda x: x[: self.max_new_categories_to_show]
            )
    ...
    def reduce_output(self, check_result: CheckResult) -> Dict[str, float]:
        ...
        feature_importance = (
            check_result.value["Feature importance"] if "Feature importance" in check_result.value.columns else None
        )
    ...
    def add_condition_new_category_ratio_less_or_equal(self, max_ratio: float = 0):
        ...
        def condition(result: pd.DataFrame) -> ConditionResult:
            failing = result[result["Ratio of New Categories"] > max_ratio]["Ratio of New Categories"].apply(
                format_percent
            )
            if len(failing) > 0:
                return ConditionResult(
                ...
                if any(result["Ratio of New Categories"] > 0):
                    ...
                    new_categories_ratio = dict(
                        new_categories_columns["Ratio of New Categories"].apply(format_percent)[:5]
                    )
