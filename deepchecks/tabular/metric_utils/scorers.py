...
try:
    ...
    from sklearn.metrics import (  # noqa: F401  pylint: disable=ungrouped-imports
        f1_score,
        jaccard_score,
        precision_score,
        recall_score,
...
SUPPORTED_MODELS_DOCLINK = doclink(
    "supported-prediction-format", template="For more information please refer to the Supported Models guide {link}"
)
...
class DeepcheckScorer:
    ...
    def __init__(
        ...
        if isinstance(scorer, str):
            formatted_scorer_name = (
                scorer.lower().replace("sensitivity", "recall").replace("specificity", "tnr").replace(" ", "_")
            )
            if formatted_scorer_name in regression_scorers_lower_is_better_dict:
                warnings.warn(
                    f"Deepchecks checks assume higher metric values represent better performance. "
            ...
            if formatted_scorer_name in _str_to_scorer_dict:
                ...
                try:
                    ...
                    raise errors.DeepchecksValueError(
                        f"Scorer name {scorer} is unknown. " f"See metric guide for a list of allowed scorer names."
                    ) from e
            ...
            def predict_proba(self, data: pd.DataFrame) -> np.ndarray:
                ...
                if not hasattr(self.user_model, "predict_proba"):
                    if isinstance(self.user_model, ClassifierMixin):
                        raise errors.DeepchecksValueError(
                            "Model is a sklearn classification model but lacks the"
                            " predict_proba method. Please train the model with"
                            " probability=True."
                        )
                    raise errors.DeepchecksValueError(
                        f"Scorer requires predicted probabilities, either implement "
    ...
    def _run_score(self, model, data: pd.DataFrame, label_col: pd.Series):
        ...
        if self.model_classes is not None:
            ...
            if model.is_binary:
                if len(label_col.unique()) > 2:
                    raise errors.DeepchecksValueError(
                        "Model is binary but the label column has more than 2 classes: " f"{label_col.unique()}"
                    )
        ...
        if self.model_classes is not None and isinstance(scores, np.ndarray):
            ...
            if (
                model.is_binary
                and len(original_label_col.unique()) == 1
                and len(model.predictions.unique()) == 1
                and original_label_col[0] == model.predictions[0]
            ):
                seen_class = original_label_col[0]
                unseen_class = self.model_classes[0] if seen_class == self.model_classes[1] else self.model_classes[1]
    ...
    def validate_fitting(self, model, dataset: "tabular.Dataset"):
        ...
        if isinstance(result, dict):
            ...
            if len(result) != len(all_classes):
                raise errors.DeepchecksValueError(
                    f"Expected {len(all_classes)} classes, but scorer {self.name} "
                    f"returned {len(result)} elements in the score array value"
                )
        elif result is not None and not isinstance(result, Number):
            raise errors.DeepchecksValueError(
                f"Expected scorer {self.name} to return number or dict " f"but got: {type(result).__name__}"
            )
...
def init_validate_scorers(
    scorers: t.Union[t.Mapping[str, t.Union[str, t.Callable]], t.List[str]],
    model: BasicModel,
...
def _transform_to_multi_label_format(y: np.ndarray, classes):
    ...
    if y.ndim == 1:
        kwargs = (
            {"sparse_output": False} if version.parse(scikit_version) >= version.parse("1.2") else {"sparse": False}
        )
