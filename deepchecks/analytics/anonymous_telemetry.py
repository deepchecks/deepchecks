...
ANALYTICS_DISABLED = os.environ.get("DISABLE_DEEPCHECKS_ANONYMOUS_TELEMETRY", False) or os.environ.get(
...
def validate_latest_version():
    ...
    if not ANALYTICS_DISABLED:
        try:
            ...
            if not is_on_latest:
                get_logger().warning(
                    "You are using deepchecks version %s, however a newer version is available."
