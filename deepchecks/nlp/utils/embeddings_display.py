...
def create_performance_display(text: pd.Series, embeddings: np.ndarray, proba: np.ndarray, y_true: np.ndarray,
    ...
    loss_per_sample = [log_loss([y_true], [y_proba], labels=sorted(labels)) for
                       y_true, y_proba in zip(y_true, proba)]
    ...
    text_dataframe['correct prediction'] = [x == y for x, y in zip(text_dataframe['label'], text_dataframe['prediction'])]

    if indexes_to_highlight is not None:
        text_dataframe['highlight criteria'] = [_select_highlight_value(x, indexes_to_highlight) for x in text_dataframe.index]
    ...
    checkpoint = tf.train.Checkpoint(performance_embedding=tf.Variable(pd.DataFrame(embeddings, index=text_dataframe.index)))
...
def create_drift_display(train_text: pd.Series, test_text: pd.Series,
                         train_embeddings: np.ndarray, test_embeddings: np.ndarray,
                         path: str, sample_size: int = 2500, verbose: bool = False,
                         indexes_to_highlight: Dict[str, List[int]] = None):
    if not os.path.exists(path):
        os.makedirs(path)
