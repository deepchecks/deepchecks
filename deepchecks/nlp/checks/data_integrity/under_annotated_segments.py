...
class UnderAnnotatedSegments(SingleDatasetCheck, WeakSegmentAbstract):
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        loss_per_sample = pd.Series([1 - pd.isna(x) for x in text_data.label], index=encoded_dataset.data.index)
        ...
        avg_score = round(loss_per_sample.mean(), 3)
        ...
        return CheckResult(
            {'weak_segments_list': weak_segments, 'avg_score': avg_score, 'scorer_name': 'Annotation Ratio'},
            display=[display_msg, 'place holder for figure'])
