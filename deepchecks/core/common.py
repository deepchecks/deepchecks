...
def import_type(module_name: str, type_name: str, base: t.Optional[t.Type[t.Any]] = None) -> t.Type[t.Any]:
...
def validate_config(conf: t.Dict[str, t.Any], version_unmatch: VersionUnmatchAction = "warn") -> t.Dict[str, t.Any]:
    ...
    if "version" not in conf or not conf["version"]:
        if version_unmatch == "raise":
            raise ValueError("Configuration must contain not emtpy " '"version" key of type string')
        elif version_unmatch == "warn":
            get_logger().warning("Configuration was expected to contain not emtpy " '"version" key of type string')
