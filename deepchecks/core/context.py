        ...
        if hasattr(check, "n_samples"):
            ...
            if dataset_kind:
                ...
                if dataset.is_sampled(n_samples):
                    message = (
                        f"Data is sampled from the original dataset, running on "
                ...
                if self._train is not None and self._train.is_sampled(n_samples):
                    message += (
                        f"Running on {self._train.len_when_sampled(n_samples)} <b>train</b> data samples "
                ...
                if self._test is not None and self._test.is_sampled(n_samples):
                    if message:
                        message += " "
                    message += (
                        f"Running on {self._test.len_when_sampled(n_samples)} <b>test</b> data samples "
