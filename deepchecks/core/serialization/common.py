...
def aggregate_conditions(
    ...
    for check_result in check_results:
        for cond_result in check_result.conditions_results:
            ...
            data.append([
                icon, link, cond_result.name, cond_result.details, priority, check_header
            ])

    df = pd.DataFrame(
        data=data,
        columns=['Status', 'Check', 'Condition', 'More Info', '$priority', '$check_header']
    )

    df.sort_values(by=['$priority', '$check_header'], inplace=True)
    df.drop('$priority', axis=1, inplace=True)
    df.drop('$check_header', axis=1, inplace=True)
...
def sort_check_results(
    check_results: t.Sequence['check_types.CheckResult']
) -> t.List['check_types.CheckResult']:
    """Sort sequence of 'CheckResult' instances for the display.

    This function returns a list of check results in the order
    in which they appear on the conditions summary table generated by
    the 'aggregate_conditions' function.

    NOTE/TODO:
    This function is logically related/connected to the 'aggregate_conditions'
    function but not programmatically. Therefore if you make some changes to
    the 'aggregate_conditions' function do not forget to also modify this function.
    This is actually not good and must be fixed in the future.
    ...
    for check_index, check_result in enumerate(check_results):
        check_results_index[check_index] = check_result

        if not check_result.conditions_results:
            raise ValueError(
                'This function is only applicable to '
                'the check results with conditions'
            )

        for condition_result in check_result.conditions_results:
            priority = (
                condition_result.priority + 0.1
                if not check_result.display
                else condition_result.priority
            )
            data.append([
                check_index,
                check_result.get_header(),
                priority
            ])

    df = pd.DataFrame(
        data=data,
        columns=['index', 'check_header', 'priority']
    )
    df.sort_values(
        by=['priority', 'check_header'],
        inplace=True
    )
    return [
