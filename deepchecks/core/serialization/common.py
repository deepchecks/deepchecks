...
from deepchecks.core import check_result as check_types, errors
...
def aggregate_conditions(
    ...
    check_results = [check_results] if isinstance(check_results, check_types.CheckResult) else check_results
    ...
    for check_result in check_results:
        for cond_result in check_result.conditions_results:
            ...
            data.append([icon, link, cond_result.name, cond_result.details, priority])

    df = pd.DataFrame(data=data, columns=["Status", "Check", "Condition", "More Info", "$priority"])
...
def create_results_dataframe(
    ...
    return pd.DataFrame(data=data, columns=["Check", "Summary"])
...
def create_failures_dataframe(
    ...
    for it in failures:
        if isinstance(it, check_types.CheckResult):
            ...
            message = it.exception.html if isinstance(it.exception, errors.DeepchecksBaseError) else str(it.exception)
            error_types = (
...
def concatv_images(images, gap=10):
    ...
    try:
        ...
        raise ImportError(
            "concatv_images function requires the PIL package. " 'To get it, run "pip install pillow".'
        ) from e
    else:
