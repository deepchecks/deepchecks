...
class SuiteResultSerializer(IPythonSerializer["suite.SuiteResult"]):
    ...
    def prepare_summary(self, output_id: t.Optional[str] = None, **kwargs) -> HTML:
        ...
        HTML
        """
        return HTML(self._html_serializer.prepare_summary(output_id=output_id, **kwargs))

    def prepare_conditions_table(
        self, output_id: t.Optional[str] = None, include_check_name: bool = True, **kwargs
    ) -> HTML:
        ...
        return HTML(
            self._html_serializer.prepare_conditions_table(
                output_id=output_id, include_check_name=include_check_name, **kwargs
            )
        )
    ...
    def prepare_results_with_condition_and_display(
        ...
        **kwargs,
        ...
        results_with_condition_and_display = [
            CheckResultSerializer(it).serialize(output_id=output_id, check_sections=check_sections, **kwargs)
            for it in results
        ]
        content = join(results_with_condition_and_display, HTML(Html.light_hr))
        return list(flatten([HTML("<h2>Check With Conditions Output</h2>"), content]))
    ...
    def prepare_results_without_condition(
        ...
        **kwargs,
        ...
        content = join(results_without_conditions, HTML(Html.light_hr))
        return list(flatten([HTML("<h2>Check Without Conditions Output</h2>"), content]))
