...
class DisplayItemsHandler(ABCDisplayItemsHandler):
    ...
    def handle_display(
        ...
        **kwargs,
        ...
        if output_id is not None and include_trailing_link:
            output.append(cls.go_to_top_link(output_id, is_for_iframe_with_srcdoc=is_for_iframe_with_srcdoc))
    ...
    @classmethod
    def handle_figure(cls, item: BaseFigure, index: int, plotly_to_image: bool = False, **kwargs) -> str:
    ...
    def handle_display_map(cls, item: "check_types.DisplayMap", index: int, **kwargs) -> str:
        ...
        return "".join(
            [
                template.format(
                    name=k,
                    content="".join(cls.handle_display(v, include_header=False, include_trailing_link=False, **kwargs)),
                )
...
def verify_include_parameter(include: t.Optional[t.Sequence[CheckResultSection]] = None) -> t.Set[CheckResultSection]:
    """Verify CheckResultSection sequence."""
    sections = t.cast(t.Set[CheckResultSection], {"condition-table", "additional-output"})
    ...
    if len(sections_to_include.difference(sections)) > 0:
        raise ValueError(
            "include parameter must contain " 'Union[Literal["condition-table"], Literal["additional-output"]]'
        )
