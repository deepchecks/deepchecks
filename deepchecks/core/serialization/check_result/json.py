...
from deepchecks.core import (
    check_result as check_types,
    checks,  # pylint: disable=unused-import
)
...
class DisplayItemsHandler(ABCDisplayItemsHandler):
    ...
    @classmethod
    def handle_dataframe(cls, item: t.Union[pd.DataFrame, Styler], index: int, **kwargs) -> t.Dict[str, t.Any]:
        """Handle dataframe item."""
        if isinstance(item, Styler):
            return {"type": "dataframe", "payload": item.data.to_dict(orient="records")}
        else:
            return {"type": "dataframe", "payload": item.to_dict(orient="records")}
    ...
    def handle_display_map(cls, item: "check_types.DisplayMap", index: int, **kwargs) -> t.Dict[str, t.Any]:
        ...
        return {"type": "displaymap", "payload": {k: cls.handle_display(v, **kwargs) for k, v in item.items()}}
