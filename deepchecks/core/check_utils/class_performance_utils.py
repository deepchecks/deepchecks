...
def get_condition_test_performance_greater_than(min_score: float) -> t.Callable[[pd.DataFrame], ConditionResult]:
    ...
    def condition(check_result: pd.DataFrame):
        ...
        details += (
            f'Found minimum score for {min_metric["Metric"]} metric of value ' f'{format_number(min_metric["Value"])}'
        )
...
def get_condition_train_test_relative_degradation_less_than(
    threshold: float
    ...
    def condition(check_result: pd.DataFrame) -> ConditionResult:
        ...
        if classes is not None:
            for class_name in classes:
                ...
                diff = {
                    score_name: _ratio_of_change_calc(score, test_scores_dict[score_name])
