...
__all__ = ['ScatterPerformance']


SDP = TypeVar('SDP', bound='ScatterPerformance')


class ScatterPerformance(SingleDatasetCheck):
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        model = context.model
        scorers = context.get_scorers(self.scorers, use_avg_defaults=False)[:2]

        results_df = pd.DataFrame()
        results_df[scorers[0].name] = scorers[0](model, dataset)
        results_df[scorers[1].name] = scorers[1](model, dataset)

        display = None
        if context.with_display:
            display = px.scatter(results_df, x=scorers[0].name, y=scorers[1].name)
