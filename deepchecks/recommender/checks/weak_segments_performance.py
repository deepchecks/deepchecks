...
from deepchecks.recommender.context import _DummyModel
...
class WeakSegmentsPerformance(SingleDatasetCheck, WeakSegmentAbstract):
    """Search for segments with low performance scores based on user features.
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        # dataset = dataset.sample(min(self.n_samples,len(dataset)), random_state=self.random_state)#.drop_na_labels()
        y_pred = context.model.predictions.values
        ...
        dummy_model = _DummyModel(test=encoded_dataset, y_pred_test=y_pred,
